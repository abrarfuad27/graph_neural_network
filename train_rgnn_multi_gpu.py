...
def run_training_proc(rank, world_size,
  ...
  #loading dataset
  with EmissionsTracker(project_name='loading dataset') as tracker:
    epoch_times = []
    epoch_accuracies = []
    power_usage = []
  ...
  for epoch in tqdm.tqdm(range(epochs)):
    ...
    #batch training
    with EmissionsTracker(project_name='batch training') as tracker:
      for batch in train_loader:
        ...
        if idx % validation_freq == 0:
          ...
          epoch_accuracies.append(global_acc)
          epoch_times.append(time.time() - training_start)
    ...
    #evaluation
    with EmissionsTracker(project_name='evaluation') as tracker:
  ...
  plt.title('Graph Neural Network Validation Accuracy vs Epoch')
  plt.savefig('validation_accuracy.png')

  plt.figure()
  plt.plot(epoch_times)
  plt.xlabel('Epoch')
  plt.ylabel('Time taken to train (seconds)')
  plt.title('Graph Neural Network Training Time vs Epoch')
  ...
  plt.title('Graph Neural Network Emissions vs Epoch')
...
if __name__ == '__main__':
  ...
  #graph init
  with EmissionsTracker(project_name="graph init") as tracker:
